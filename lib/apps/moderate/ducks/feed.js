...
export default (state = {}, action) => {
  switch (action.type) {
  case FETCH:
    return assign({}, state, { [action.meta.member.id]: { fetching: true, fetched: false, error: null }});
  case SUCCESS:
    return assign({}, state, { [action.meta.member.id]: { fetching: false, fetched: true, error: null, data: action.payload.result }});
  case FAILED:
    return assign({}, state, { [action.meta.member.id]: { fetching: false, fetched: false, error: action.payload.statusText }});
